 ...
 */
export default function PhoneVerifyField({ phoneNumber, onChangePhoneNumber, verified, onVerified }) {
    ...
    <div className="field">
      ...
      <div className="verify-row">
        <input
          ...
          value={phoneNumber}
          disabled={step === 'sent'}
          onChange={(e) => onChangePhoneNumber(e.target.value.replace(/[^0-9]/g, ''))}
        />
        <button
          type="button"
          className="btn btn-secondary"
          disabled={sendLoading || (step === 'sent' && secondsLeft > 0)}
          onClick={handleSendCode}
        >
          {step === 'idle' ? '인증번호 받기' : secondsLeft > 0 ? '재전송' : '재전송'}
        </button>
      </div>
      ...
      {step === 'sent' && !error && (
        <span className="field-hint">문자로 받은 6자리 인증번호를 입력해주세요.</span>
      )}
    </div>
  )
}
